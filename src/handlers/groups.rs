...
async fn list_groups(
    ...
    if let Some(name) = &query.name {
        sql_query = sql_query.add(models::group::Column::Name.eq(name.to_owned()));
    }
...
async fn modify_group(
    ...
    let mut group_found: models::group::ActiveModel = Group::find_by_id(group_id)
        .one(conn)
        .await
        .map_err(|err| errors::ServerError::Internal(anyhow!(err)))?
        .ok_or(errors::ServerError::NotFound)?
    ...
    if let Some(name) = &body.name {
        group_found.name = Set(name.to_owned());
    }

    let group_updated: models::group::Model = group_found
        .update(conn)
        .await
        .map_err(|err| errors::ServerError::Internal(anyhow!(err)))?;
